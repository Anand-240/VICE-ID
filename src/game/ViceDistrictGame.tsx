...
import { ArrowDown, ArrowLeft, ArrowRight, ArrowUp, Eye, Gauge, MapPin, Pause, Play, Radio, RotateCcw, ShieldAlert, Smartphone, X } from 'lucide-react';
import { motion } from 'framer-motion';
import * as THREE from 'three';
import { useCharacterStore } from '../store/characterStore';
import type { DistrictOutcome } from '../types/character';
import { NeonHarborScene, type ControlState } from './NeonHarborScene';
...
export function ViceDistrictGame({ onContinue, onReturn }: Props) {
  ...
  const resetGame = () => {
    ...
    setElapsed(0); setRecognitions(0); setPoliceReports(0); setAwareness(Math.min(38, 8 + c.wantedLevel * 5)); setPursuitActive(false); setPursuitPrompt(false); setEscapeProgress(0); setCaptured(false); setLocalBuzz(cityState?.buzz || 50); setSummary(false); setPaused(false); setPosterOpen(false); setNearPoster(false); setIntro(true);
  ...
  return <main className="district-game"><div className="game-viewport-3d" style={{ '--district-accent': theme.accent } as React.CSSProperties} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerCancel={handlePointerUp}>
    ...
    {!loading && !intro && tutorial && <div className="game-tutorial"><button aria-label="Start exploring district" onClick={() => setTutorial(false)}><X /></button><p>ENTERING {c.district.toUpperCase()}</p><div><span><kbd>WASD</kbd><b>MOVE</b></span><span><kbd>DRAG</kbd><b>CAMERA</b></span><span><kbd>SHIFT</kbd><b>RUN</b></span><span><kbd>E</kbd><b>INTERACT</b></span></div><small>YOUR EDITED POSTER IS NOW A LIVE CITY ASSET</small></div>}
